...
DYMOLA_STATIC void AssertModelicaF3(int b,const char*bs,const char*x,int lev) {
	if (!b) {
		if (lev==1) {
...
static double pow__(const char* sx, double x, const char* sy, double y)
{
  return (x == 0.0 && y <= 0.0) || (x < 0.0 && floor(y) != y) ?
    powerr(sx, x, sy, y) : pow(x, y);
}

#define pow(x,y) pow__(#x, (x), #y, (y))
#define powmacro(x,sx,y,sy) pow__(sx,x,sy,y)
...
LIBDS_API  int dymli1_(int* sysnr, const int *fact, double* a, const int* lda, const int* n, 
                   double* b, double *Time, int* Event, int* PrintEvent,
                   double* dwork, int* iwork, int* ierr);

LIBDS_API int dymli2_(int* sysnr, int *fact, double *a, const int* lda, const int *n,
		   double* b, double *Time, int *Event, int* PrintEvent,
		   double* dwork, int* iwork, int *factor, int* ierr);

LIBDS_API  int dymli3_(int* sysnr, int *fact, double *a, const int* lda, const int *n,
		   double* b, double *Time, int *Event, int* PrintEvent,
		   double* dwork, int* iwork, int *factor, const char*const*varnames,int* ierr);

LIBDS_API  int dymli4_(int* sysnr, int *fact, double *a, const int* lda, const int *n,
		   double* b, double *Time, int *Event, int* PrintEvent,
		   double* dwork, int* iwork, int *factor, const char*const*varnames,int* ierr,int*fEvent);


LIBDS_API  int dymlin_(int* infrev, const int* n, double* sol, double* res, double *jac,
