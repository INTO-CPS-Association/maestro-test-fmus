...
#define COPY_VARIABLES(source, target)                                                                    \
		...
		{                                                                                                 \
		  ...
		  for(i = 0; i< target->nSPar; ++i){                                                              \
			...
			target->sParameters[MAX_STRING_SIZE]='\0';                                                    \
		  }                                                                                               \
		}
...
DYMOLA_STATIC void fmu_logger(const char* message, int newline, int severity) 
{
	...
	switch (severity){
		case 0: stat = FMIWarning; break; /*Message from dymola should be handled as warning*/
...
DYMOLA_STATIC FMIStatus fmiGetString_(FMIComponent c, const FMIValueReference vr[], size_t nvr, FMIString value[])
{
	...
	if (comp->icall < idemand || comp->mStatus == modelInitializationMode ) {       
		/* refresh cache */                                                         
		int QiErr = 0;                                                             
        QiErr = util_refresh_cache(comp, idemand, NULL, NULL);
		if (QiErr != 0) {
													 ...
													 FMIReal dz[])
{
	...
	if (QJacobianDefined_) {
		...
		if (jacData->nJacA < nA) {
			...
			jacData->jacA = allocateMemory(nA, sizeof(double));
		}
		if (jacData->nJacB < nB) {
			freeMemory(jacData->jacB);
			jacData->jacB = allocateMemory(nB, sizeof(double));
		}
		if (jacData->nJacC < nC) {
			freeMemory(jacData->jacC);
			jacData->jacC = allocateMemory(nC, sizeof(double));
		}
		if (jacData->nJacD < nD) {
			freeMemory(jacData->jacD);
			jacData->jacD = allocateMemory(nD, sizeof(double));
		}

		if (nA && jacData->jacA == NULL || nB && jacData->jacB == NULL || nC &&jacData->jacC== NULL || nD && jacData->jacD == NULL) {
...
done1:
	...
	/*  analytic Jacobian not available */
	{
		...
		if (jacData->nJacV < nv) {
			...
			jacData->jacV = allocateMemory(nv, sizeof(FMIReal));
		}
		if (jacData->nJacZ < nz) {
			freeMemory(jacData->jacZ);
			freeMemory(jacData->jacZTmp);
			jacData->jacZ = allocateMemory(nz, sizeof(FMIReal));
			jacData->jacZTmp = allocateMemory(nz, sizeof(FMIReal));
		}

		if (jacData->jacV == NULL || jacData->jacZ == NULL || jacData->jacZTmp == NULL) {
			util_logger(comp, comp->instanceName, FMIError, "", "%s: memory allocation failed", label);
			status = FMIError;
			goto done2;
		}
		...
		for (j = 0; j < nv; j++) {
			...
			if (dv[j] != 0) {
				...
				dy  = ((jacV[j] > 0 ? 1e-3 : -1e-3) + jacV[j]) * 1e-8;
				new_value = jacV[j] + dy;
				dy = new_value - jacV[j];
				if ((status = fmiSetReal_(c, &v_ref[j], 1, &new_value)) != FMIOK ||
					(status = fmiGetReal_(c, z_ref, nz, jacZTmp)) != FMIOK) {
						goto done2;
				}
				/* Compute column j */
				for (i = 0; i < nz; i++) {
					dz[i] += dv[j] * (jacZTmp[i] - jacZ[i]) / dy;
				}
				/* Restore */
				if ((status = fmiSetReal_(c, &v_ref[j], 1, &jacV[j])) != FMIOK) {
					goto done2;
				}
			}
		}
