...
{
	integers[ID_NUMBER2]=1000000000;
																		 ...
																		 const fmi2Integer value[])
{
	...
	for(int i; i < nvr ; i++)
		{
			printf("Setting vr=%d, i=%d, val=%d\n",vr[i],i,value[i]);
			integers[vr[i]] = value[i];
		}
																 ...
																 fmi2Boolean noSetFMUStatePriorToCurrentPoint)
{
	...
	number1=integers[ID_NUMBER1];
	number2=integers[ID_NUMBER2];


		if (g_functions != NULL && g_functions->logger != NULL)
		{
			//		g_functions->logger((void*) 2, name->c_str(), fmi2OK, "logAll","doStep from %d , to %d %s\n", number1, number2, "");
		}
