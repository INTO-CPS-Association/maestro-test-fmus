/* External utility functions for Modelica packages
   ...
   to define the system calls of the operating system
    ...
    Release Notes:
      January 2013: DS, Added ModelicaV* routines here.
...
static void ModelicaNotExistError(const char* name) {
  ...
  static void ModelicaInternal_getenv(const char* name, int convertToSlash, const char**content, int* exist) {
...
#if defined(_WIN32) && (defined(_MSC_VER) || defined(__GNUC__) && defined(DYMOSIM) || defined(__WATCOMC__))
...
#elif defined(_POSIX_) || defined(_WIN32) && defined(__GNUC__) && defined(DYMOSIM) || defined(__WATCOMC__)
...
static void ModelicaInternal_readDirectory(const char* directory, int nFiles, 
  ...
  */
  #if defined(_WIN32) && (defined(_MSC_VER)||defined(__GNUC__)&&defined(DYMOSIM)||defined(__WATCOMC__)) || defined(_POSIX_)
     int errnoTemp;
     int iFiles  = 0;
     char *pName;
     struct dirent *pinfo;
...
static int ModelicaInternal_getNumberOfFiles(const char* directory) {
    ...
    pdir = opendir(directory);
    if ( pdir == NULL ) goto ERROR_Dymola2;
    errno = 0;
    while ( (pinfo = readdir(pdir)) != NULL ) {
        if ( (strcmp(pinfo->d_name, "." ) != 0) &&
            (strcmp(pinfo->d_name, "..") != 0) ) {
    ...
    errnoTemp = errno;
    closedir(pdir);
    if ( errnoTemp != 0 ) {errno = errnoTemp; goto ERROR_Dymola2;}

    return nFiles;
    
ERROR_Dymola2: ModelicaFormatError("Not possible to get number of files in \"%s\":\n%s",
                           directory, strerror(errno));
       return 0;
#else
...
static const char* ModelicaInternal_fullPathName(const char* name)
{
    ...
    if (name[0] != '/') {
        /* Any name beginiing with "/" is regarded as already being a full path. */
        strcpy(fullName, cwd);
        strcat(fullName, "/");
    }
  ...
  /* Write string to terminal or to file */
     ...
     if ( fileName[0] == '\0' ) {
        ...
        /* Write string to file */
           ...
           if ( fputs(string,fp) < 0 ) goto ERROR_Dymola2;
           if ( fputs("\n",fp)   < 0 ) goto ERROR_Dymola2;
           fclose(fp);
           return;

           ERROR_Dymola2: fclose(fp);
...
static const char* ModelicaInternal_readLine(const char* fileName, int lineNumber, int* endOfFile) {
  ...
  /* Read line lineNumber */
     line = ModelicaAllocateStringWithErrorReturn(lineLen);
     if ( line == NULL ) goto ERROR_Dymola2;

	 if (lineLen<=sizeof(localbuf)) {
	   size_t i;
	   for(i=0;i<lineLen;++i) line[i]=localbuf[i];
	 } else {
	    if ( fseek(fp, offset, SEEK_SET) != 0 ) goto ERROR_Dymola2;
        if ( fread(line, sizeof(char), lineLen, fp) != lineLen ) goto ERROR_Dymola2;
