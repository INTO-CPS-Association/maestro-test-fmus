...
#endif /* FMI_2 */
{
	...
	jacData->jacA = jacData->jacB = jacData->jacC = jacData->jacD = jacData->jacV = jacData->jacZ = jacData->jacZTmp = NULL;
...
DYMOLA_STATIC void fmiFreeModelInstance_(FMIComponent c)
{
	...
	freeMemory(comp->dstruct);
	freeMemory(comp->istruct);

	freeMemory(comp->states);
	freeMemory(comp->derivatives);
	freeMemory(comp->parameters);
	freeMemory(comp->inputs);
	freeMemory(comp->outputs);
	freeMemory(comp->auxiliary);
	freeMemory(comp->crossingFunctions);
	for(i= (int) comp->nSPar-1; i >=0; --i){
		freeMemory( (comp->sParameters)[i]);
	}
	freeMemory(comp->sParameters);
	freeMemory(comp->statesNominal);
	freeMemory(comp->oldStates);
